<?php
session_start();
if(!isset($_SESSION['loggedinaccount'])){
    header('location: index.php');
    exit;
}
$dbhost='localhost';
$dbuser='root';
$dbpassword='';
$dbname='users';
$con = mysqli_connect($dbhost, $dbuser, $dbpassword, $dbname);


if(mysqli_connect_errno()){ exit('Failed to connect to MySQL'); }


$stmt= $con->prepare('SELECT name, surname, age, password, email FROM details WHERE id=?');
$stmt->bind_param('i', $_SESSION['accountid']);
$stmt->execute();
$stmt->bind_result($name, $surname, $age, $password, $email);
$stmt->fetch();
$stmt->close();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Profile</title>
    <link rel="stylesheet" href="main3.css">
</head>
<body>
<header>
        <div class="container">
            <div id="logo"> Detyra 3</div>
            <nav>
                <ul class="nav-links">
                    <a href="profile.php"> Profile </a>
        <a href="logout.php"> <svg style="background-color: white;"width="12" height="12" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"> <path d="M377.9 105.9L500.7 228.7c7.2 7.2 11.3 17.1 11.3 27.3s-4.1 20.1-11.3 27.3L377.9 406.1c-6.4 6.4-15 9.9-24 9.9c-18.7 0-33.9-15.2-33.9-33.9l0-62.1-128 0c-17.7 0-32-14.3-32-32l0-64c0-17.7 14.3-32 32-32l128 0 0-62.1c0-18.7 15.2-33.9 33.9-33.9c9 0 17.6 3.6 24 9.9zM160 96L96 96c-17.7 0-32 14.3-32 32l0 256c0 17.7 14.3 32 32 32l64 0c17.7 0 32 14.3 32 32s-14.3 32-32 32l-64 0c-53 0-96-43-96-96L0 128C0 75 43 32 96 32l64 0c17.7 0 32 14.3 32 32s-14.3 32-32 32z"/></svg>
        Logout </a>
                </ul>
            </nav>
        </div>
    </header> <br> <br> <br>
    <div style="padding-left: 10px;">
        <strong> Welcome</strong>, <?=htmlspecialchars($name)?>! <br>
        <strong> Email: </strong>
        <?=htmlspecialchars($email)?> <br>
        <strong> Age:</strong>
        <?=htmlspecialchars($age)?> <br>
        <strong> Surname: </strong>
        <?=htmlspecialchars($surname)?>






        

    </div>
</body>
</html>
